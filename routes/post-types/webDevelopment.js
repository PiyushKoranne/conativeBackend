var express = require("express");
var router = express.Router();
var multer = require("multer");
var session = require("express-session");
var checkLogin = require("../../middleware/check");
var { upload, url, frontend_url } = require("../constants");

var MongoClient = require("mongodb").MongoClient;

var { ObjectId } = require("mongodb");


router.get("/web-development", checkLogin, async function (req, res, next) {
	let client;
	try {
		client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
		const dbo = client.db("conative");
		let websiteDesignarr = await dbo.collection("websiteDesign").findOne({});
		let webDevelopmentPosts = await dbo
			.collection("webDevelopmentPost")
			.find()
			.sort({ _id: -1 })
			.toArray();
		let userlogin = await dbo
			.collection("users")
			.findOne({ _id: new ObjectId(session.userid) });
		let headermenu_dynamic = await dbo
			.collection("menus")
			.find({
				$and: [{ $or: [{ displaymenu: "b" }] }, { $or: [{ parent_id: "1" }] }],
			})
			.sort({ index: 1 })
			.toArray();
		let setting_dynamic = await dbo
			.collection("menus")
			.find({
				$and: [{ $or: [{ displaymenu: "b" }] }, { $or: [{ parent_id: "2" }] }],
			})
			.sort({ index: 1 })
			.toArray();

		const result = await dbo.collection("option").findOne({});
		if (result) {
			session.message = "";
			res.render("admin/post-types/webDevelopmentPost", {
				title: "Web Development Post",
				headermenu: headermenu_dynamic,
				settingmenu: setting_dynamic,
				userlogin: userlogin,
				pagedata: websiteDesignarr,
				webDevelopmentPosts: webDevelopmentPosts,
				opt: result,
				frontend_url: frontend_url,
				msg: session.message,
			});
		} else {
			res.status(400).json({ message: "Data not found" })
		}

	} catch (error) {
		console.log("OPTION SHOW ROUTE ERROR\n")
		console.log(error);
		res.status(500).json({ message: "Some error occurred", error: error?.message })
	} finally {
		// Close the database connection in the finally block
		if (client) {
			client.close();
		}
	}
});

router.post("/website-design-content", async function (req, res, next) {
	let client;
	try {
		client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
		const dbo = client.db("conative");
		const file = req.file;

		var imagepath = "";
		if (req.body.oldimage != "") {
			imagepath = req.body.oldimage;
		}
		if (file && !file.length) {
			imagepath = file.filename;
		}

		var myobj = {
			oid: "1",
			meta_title: req.body.meta_title.trim(),
			meta_desc: req.body.meta_desc.trim(),
			meta_keywords: req.body.meta_keywords.trim(),
			title: req.body.title.trim(),
			heading: req.body.heading.trim(),
			content: req.body.content.trim(),
		};
		const result = await dbo
			.collection("websiteDesign")
			.findOneAndUpdate(
				{ oid: "1" },
				{ $set: myobj },
				{ upsert: true, returnNewDocument: true }
			);
		if (result) {
			session.message = "Data updated successfully";
			return res.redirect("/web-development");
		} else {
			session.message = "Failed to update data";
			return res.redirect("/web-development");
		}

	} catch (error) {
		console.log("OPTION SHOW ROUTE ERROR\n")
		console.log(error);
		res.status(500).json({ message: "Some error occurred", error: error?.message })
	} finally {
		// Close the database connection in the finally block
		if (client) {
			client.close();
		}
	}
});

router.post(
	"/web-development-post",
	upload.array("design_image[]", 30),
	async function (req, res, next) {
		var response = [];
		if (
			req.body.meta_title == "" ||
			req.body.meta_desc == "" ||
			req.body.meta_keywords == "" ||
			req.body.title == "" ||
			req.body.post_content == "" ||
			req.body.slug == "" ||
			req.body.design_type == "" ||
			req.body.uploaded_images == ""
		) {
			response["error"] = "Please fill all the required fields";
			session.message = response;
			// return res.redirect("/web-development");
		}
		let client;
		try {
			client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
			const dbo = client.db("conative");
			const date = new Date();

			var month = date.getMonth() + 1;
			let year = date.getFullYear();

			const uniqueSuffix = year + "-" + month + "_";

			const files = req.files;
			const uploaded_images = req.body.uploaded_images;

			var file_names = [];

			if (req.body.editid != "") {
				for (var item of uploaded_images) {
					file_names.push(item);
				}
			}

			for (var item of files) {
				file_names.push(item.filename);
			}

			if (req.body.editid != "") {
				var myobj = {
					$set: {
						meta_title: req.body.post_meta_title.trim(),
						meta_desc: req.body.post_meta_desc.trim(),
						meta_keywords: req.body.post_meta_keywords.trim(),
						title: req.body.title.trim(),
						post_content: req.body.post_content.trim(),
						slug: req.body.slug.trim(),
						design_type: req.body.design_type,
						images: file_names,
					},
				};

				var result = await dbo.collection("webDevelopmentPost").updateOne(
					{ _id: new ObjectId(req.body.editid) },
					myobj);
				if (result) {
					response["success"] = "webDevelopmentPost updated successfully";
					session.message = response;
				} else {
					response["success"] = "failed to update webDevelopmentPost";
					session.message = response;
				}

			} else if (req.body.editid == "") {
				var myobj = {
					meta_title: req.body.post_meta_title.trim(),
					meta_desc: req.body.post_meta_desc.trim(),
					meta_keywords: req.body.post_meta_keywords.trim(),
					title: req.body.title.trim(),
					post_content: req.body.post_content.trim(),
					slug: req.body.slug.trim(),
					design_type: req.body.design_type,
					images: file_names,
				};

				let result = await dbo
					.collection("webDevelopmentPost")
					.insertOne(myobj);

				if (result) {
					response["success"] = "webDevelopmentPost added successfully";
					session.message = response;
					return res.redirect("/web-development");
				} else {
					response["success"] = "failed to add webDevelopmentPost";
					session.message = response;
					return res.redirect("/web-development");
				}
				// return res.redirect("/web-development");
			}

		} catch (error) {
			console.log("OPTION SHOW ROUTE ERROR\n")
			console.log(error);
			res.status(500).json({ message: "Some error occurred", error: error?.message })
		} finally {
			// Close the database connection in the finally block
			if (client) {
				client.close();
			}
		}
	}
);

router.get("/web-development-post-show", async function (req, res, next) {
	let client;
	try {
		client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
		const dbo = client.db("conative");
		
		const result = await dbo
								.collection("webDevelopmentPost")
								.find()
								.sort({ _id: -1 })
								.toArray({});
		if (result) {
			res.status(200).json(result)
		} else {
			res.status(400).json({ message: "Data not found" })
		}

	} catch (error) {
		console.log("OPTION SHOW ROUTE ERROR\n")
		console.log(error);
		res.status(500).json({ message: "Some error occurred", error: error?.message })
	} finally {
		// Close the database connection in the finally block
		if (client) {
			client.close();
		}
	}
});

router.get(
	"/web-development-post-single/:slug",
	async function (req, res, next) {
		let client;
		try {
			client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
			const dbo = client.db("conative");
			if (!req.params.slug) {
				res.status(404).json({ err: "slug not found" });
				process.exit(1);
			}
			const result = await dbo
									.collection("webDevelopmentPost")
									.findOne({ slug: req.params.slug });
			if (result) {
				res.status(200).json(result)
			} else {
				res.status(400).json({ message: "Data not found" })
			}

		} catch (error) {
			console.log("OPTION SHOW ROUTE ERROR\n")
			console.log(error);
			res.status(500).json({ message: "Some error occurred", error: error?.message })
		} finally {
			// Close the database connection in the finally block
			if (client) {
				client.close();
			}
		}
	}
);

router.get("/getWebDevelopmentPost/:id", async function (req, res, next) {
	let client;
	try {
		client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
		const dbo = client.db("conative");
		var aid = req.params.id;
		const result = await dbo
								.collection("webDevelopmentPost")
								.findOne({ _id: new ObjectId(aid) });
		if (result) {
			res.status(200).json(result)
		} else {
			res.status(400).json({ message: "Data not found" })
		}

	} catch (error) {
		console.log("OPTION SHOW ROUTE ERROR\n")
		console.log(error);
		res.status(500).json({ message: "Some error occurred", error: error?.message })
	} finally {
		// Close the database connection in the finally block
		if (client) {
			client.close();
		}
	}
});

router.get("/webDevelopmentPost-remove/:id", async function (req, res, next) {
	let client;
	try {
		client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
		const dbo = client.db("conative");
		const result = await dbo.collection("option").findOne({});
		if (result) {
			session.massage = "webDevelopmentPost deleted successfully";
			res.redirect("/web-development");
		} else {
			session.massage = "failed to delete webDevelopmentPost";
			res.redirect("/web-development");
		}

	} catch (error) {
		console.log("OPTION SHOW ROUTE ERROR\n")
		console.log(error);
		res.status(500).json({ message: "Some error occurred", error: error?.message })
	} finally {
		// Close the database connection in the finally block
		if (client) {
			client.close();
		}
	}
});

router.get("/website-design-content-show", async function (req, res, next) {
	let client;
	try {
		client = await MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });
		const dbo = client.db("conative");
		
		const result = await dbo.collection("websiteDesign").findOne({});
		if (result) {
			res.status(200).json(result)
		} else {
			res.status(400).json({ message: "Data not found" })
		}

	} catch (error) {
		console.log("OPTION SHOW ROUTE ERROR\n")
		console.log(error);
		res.status(500).json({ message: "Some error occurred", error: error?.message })
	} finally {
		// Close the database connection in the finally block
		if (client) {
			client.close();
		}
	}
});

module.exports = router;
