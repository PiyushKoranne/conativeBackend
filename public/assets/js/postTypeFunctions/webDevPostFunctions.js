const uploaderElements = document.querySelectorAll(".posttype-web-development-uploader");
uploaderElements.forEach(element => {
	element.addEventListener("change", readURL);
})

const editElement = document.querySelector("#posttype-web-development-edit");
editElement.addEventListener("click", webDevelopmentPost_edit);


function webDevelopmentPost_edit(event){
	const editid = event.target.dataset.identifier;
	// alert(editid);
	  $.ajax({
	  url:"/getWebDevelopmentPost/"+editid,
	  method:"GET",
